package com.example.colspeak.pronunciation;
...
public class PronunciationEvaluator {
    ...
    private static final String TAG = "PronunciationEvaluator";
     ...
     */
    public PronunciationEvaluator(Context context) {
        ...
        speechRecognizer.setRecognitionListener(new RecognitionListener() {
            ...
            public void onResults(Bundle results) {
                ...
                if (matches != null && !matches.isEmpty()) {
                    ...
                    // En modo online, normalizar números a palabras para evitar transcripciones con dígitos ("23" -> "twenty three")
                    if (!isOfflineMode) {
                        spokenText = convertNumbersToWordsInText(spokenText);
                    }
                        ...
                        if (isCorrect) {
                            ...
                            if (dbHelper != null) {
                                long resultId = dbHelper.savePronunciationResult(
                                    ...
                                    spokenText,
                                    score,
                                    "General",  // Tema por defecto
                                    "A1.1"     // Nivel por defecto
                                );
                                if (resultId != -1) {
                            ...
                            if (callback != null) {
                                callback.onResult(score, spokenText);
                            }
                        } else {
            ...
            public void onPartialResults(Bundle partialResults) {
                ...
                if (matches != null && !matches.isEmpty()) {
                    ...
                    if (!isOfflineMode) {
                        partialText = convertNumbersToWordsInText(partialText);
                    }
                    if (callback != null) {
    ...
    private void syncPronunciationResult(long resultId, String referenceText, String spokenText, double score) {
        ...
        if (!isOfflineMode) {
            try {
                // Sincronización con Firebase deshabilitada: dependencias y referencias removidas
                // Mantener registro para diagnóstico
                Log.d(TAG, "Sync skipped: Firebase disabled. ResultId=" + resultId);
            } catch (Exception e) {
    ...
    public void startListening(PronunciationCallback callback) {
        ...
        if (referenceText == null) {
            ...
            return;
                ...
                // Iniciar reconocimiento inmediatamente sin delay
        ...
        // Modo online: usar configuración estándar sin modificaciones especiales
    ...
    public double evaluatePronunciation(String spokenText) {
        ...
        // Normalizar números a palabras en modo online antes de evaluar
        if (!isOfflineMode) {
            spokenText = convertNumbersToWordsInText(spokenText);
        }
        
        // Lógica original para palabras normales
        ...
        // Detectar si es un caso de letras del alfabeto separadas
        boolean isAlphabetCase = isAlphabetLettersCase(cleanReference);
        
        if (isAlphabetCase && !isOfflineMode) {
            // Modo online con letras del alfabeto: usar evaluación especial
            return evaluateAlphabetPronunciation(cleanReference, cleanSpoken);
        }
        ...
        // Configuración unificada para ambos modos (online y offline)
        // Aplicar la misma metodología tolerante para ambos
        double finalScore = (wordScore * 0.6) + (charScore * 0.3) + (lengthScore * 0.1);

        // Penalización por palabras incorrectas - más tolerante para ambos modos
        int incorrectWords = Math.abs(referenceTokens.length - spokenTokens.length);
        if (incorrectWords > 0) {
            // Penalización reducida para ser más tolerante
            finalScore *= (1.0 - (incorrectWords * 0.05));
        }
    ...
    // Conversión básica de números a palabras en inglés para mejorar evaluación en modo online
    private String convertNumbersToWordsInText(String text) {
        ...
        for (int i = 0; i < tokens.length; i++) {
            ...
            if (token.matches("\\d+")) {
                try {
                    ...
                    normalized = numberToWords(value);
    ...
    // Soporte básico: 0..99999 (suficiente para NUMBERS A1)
    private String numberToWords(long n) {
        ...
        if (n >= 100) {
            sb.append(below20[(int)(n / 100)]).append(" hundred");
            n %= 100;
            if (n > 0) sb.append(' ');
        }
        if (n >= 20) {
            ...
            n %= 10;
            if (n > 0) sb.append(' ').append(below20[(int)n]);
        } else if (n > 0) {
            sb.append(below20[(int)n]);
        }
        return sb.toString().trim();
    }

    /**
     * Detecta si el texto de referencia contiene letras del alfabeto separadas por espacios
     */
    private boolean isAlphabetLettersCase(String text) {
        String[] tokens = text.split("\\s+");
        if (tokens.length < 2) return false;
        
        // Verificar si todos los tokens son letras individuales
    ...
    /**
     * Evaluación especial para letras del alfabeto en modo online
     * Maneja el caso donde Google Speech Recognition pega las letras
     */
    private double evaluateAlphabetPronunciation(String reference, String spoken) {
        // Remover espacios del texto de referencia para comparar
        String referenceNoSpaces = reference.replaceAll("\\s+", "");
        
        // Si el texto hablado no tiene espacios, comparar directamente
        if (!spoken.contains(" ")) {
            return calculateCharacterSimilarity(referenceNoSpaces, spoken);
        }
        
        // Si tiene espacios, usar evaluación normal pero más tolerante
        String[] referenceTokens = reference.split("\\s+");
        String[] spokenTokens = spoken.split("\\s+");
        
        double wordScore = calculateDetailedWordScore(referenceTokens, spokenTokens);
        double charScore = calculateCharacterSimilarity(reference, spoken);
        double lengthScore = calculateLengthScore(referenceTokens, spokenTokens);
        
        // Para letras del alfabeto, ser más tolerante con la longitud
        double finalScore = (wordScore * 0.5) + (charScore * 0.4) + (lengthScore * 0.1);
        
        // Penalización muy reducida para letras del alfabeto
        int incorrectWords = Math.abs(referenceTokens.length - spokenTokens.length);
        if (incorrectWords > 0) {
            finalScore *= (1.0 - (incorrectWords * 0.02)); // Solo 2% de penalización
        }
                ...
                for (int j = 0; j < reference.length; j++) {
                    if (!matchedReference[j] && !matchedSpoken[i]) {
                        ...
                        if (isOfflineMode) {
                            ...
                            //if (similarity > 0.6) {
                            if (similarity > 0.5) { // Umbral ajustado de 0.6 a 0.5
                                matchedReference[j] = true;
                                matchedSpoken[i] = true;
                                partialWords++;
                                break;
    ...
    private double calculateCharacterSimilarity(String s1, String s2) {
        ...
        if (isOfflineMode) {
            ...
            if (maxLength - minLength > 5) { // Umbral ajustado de 3 a 5
                return 0.0;
            }
        } else {
            // Modo online: mantener igual que estaba
    ...
    private double calculateWordSimilarity(String word1, String word2) {
        ...
        if (isOfflineMode) {
            ...
            if (maxLength - minLength > 3) { // Umbral ajustado de 2 a 3
                return 0.0;
            }
        } else {
            // Modo online: mantener igual que estaba
        ...
        double lengthPenalty = (maxLength - minLength) * 0.05; // Penalización reducida de 0.1 a 0.05
    ...
    private String getErrorText(int errorCode) {
        switch (errorCode) {
            case SpeechRecognizer.ERROR_AUDIO:
                return "Error de audio - Intenta hablar más cerca del micrófono";
            case SpeechRecognizer.ERROR_CLIENT:
                return "Error del cliente - Intenta de nuevo";
            case SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS:
                return "Permisos insuficientes - Verifica los permisos del micrófono";
            case SpeechRecognizer.ERROR_NETWORK:
                return "Error de red - Verifica tu conexión a internet";
            case SpeechRecognizer.ERROR_NETWORK_TIMEOUT:
                return "Tiempo de espera de red agotado - Intenta de nuevo";
            case SpeechRecognizer.ERROR_NO_MATCH:
                // Mensaje más amigable para el modo online
                if (!isOfflineMode) {
                    return "No se pudo reconocer - Intenta hablar más claro o más cerca del micrófono";
                } else {
                    return "No se encontró coincidencia";
                }
            case SpeechRecognizer.ERROR_RECOGNIZER_BUSY:
                return "Reconocedor ocupado - Espera un momento e intenta de nuevo";
            case SpeechRecognizer.ERROR_SERVER:
                return "Error del servidor - Intenta de nuevo en unos momentos";
            case SpeechRecognizer.ERROR_SPEECH_TIMEOUT:
                return "Tiempo de espera agotado - Habla más rápido o más cerca del micrófono";
            default:
